...
Current source、test、workflowに対するSpec IDの索引。Line coverage率ではない。
...
| INST-004 | `install/service.go` | bulk preflight/partial failure tests |
...
All external runtime paths and persisted fields map to an ID. Private helpers share their owner ID.
